...
class Moderation(commands.Cog):
    ...
    @commands.command(
        description="softban a user", usage="softban [user] <reason>", aliases=["sb"]
    )
    @commands.haspermissions(ban_members=True)
    @has_permissions(ban_members=True)
    @commands.cooldown(1, 5, commands.BucketType.user)
    async def softban(self, ctx: Context, *, user: discord.User):
        ...
        if data and data["message"]:
            ...
            if content or embed:
                ...
                await ctx.channel.send(content=processed_message)
        if data is None:
            return await ctx.approve(f"softbanned {user.mention}")
        await send_modlog(
            self.bot, "softbanned", ctx.author, user, reason="softbanned user."
        )
    ...
    async def nuke(self, ctx: Context, *, channel: discord.TextChannel = None):
        ...
        await send_modlog(
            self.bot, "channel nuked", ctx.author, channel, reason="Channel nuked."
        )
...
class Confirm(discord.ui.View):
    ...
    async def confirm(
        ...
        embed.set_footer(
            text=f"Nuked by {self.ctx.author}", icon_url=self.ctx.author.avatar.url
        )
