...
class Owner(Cog):
    ...
    @hybrid_command(
        name="say", aliases=["repeat", "rp"], description="Make the bot repeat the text"
    )
    @is_owner()
    ...
    @commands.command(
        name="unblacklist", aliases=["unbl"], description="Unblacklist a user."
    )
    @commands.is_owner()
    async def unblacklist(
        self, ctx: Context, *, user: Union[discord.User, discord.Member]
    ):
    ...
    @commands.command(name="blacklist", aliases=["bl"], description="Blacklist a user.")
    @commands.is_owner()
    async def blacklist(
        self, ctx: Context, *, user: Union[discord.User, discord.Member]
    ):
    ...
    @command(name="blacklistguild", description="Blacklists a guild.")
    @commands.is_owner()
    async def blacklistguild(self, ctx: Context, *, guildid: int):
        ...
        check = await self.bot.pool.fetchrow(
            "SELECT 1 FROM blacklistguild WHERE guild_id = $1", guildid
        )
        if check:
            await self.bot.pool.execute(
                "DELETE FROM blacklistguild WHERE guild_id = $1", guildid
            )
            await ctx.approve(f"Guild {guildid} has been removed from the blacklist.")
        else:
            await self.bot.pool.execute(
                "INSERT INTO blacklistguild (guild_id) VALUES ($1)", guildid
            )
    ...
    async def upload(self, ctx, attachment: discord.Attachment):
        ...
        files = {"fileToUpload": (attachment.filename, file_bytes)}
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(url, data=form) as response:
                if response.status == 200:
                    ...
                    await ctx.send(
                        f"Failed to upload the file. Status Code: {response.status}"
                    )

    @commands.Cog.listener()
    async def on_guild_join(self, guild):
        check = await self.bot.pool.fetchrow(
            "SELECT 1 FROM blacklistguild WHERE guild_id = $1", guild.id
        )

        if check:
            await guild.leave()
            owner = await self.bot.fetch_user()  # edit with your own id if you want
    ...
    async def gi(self, ctx, identifier: str):
        ...
        if guild:
            invite = await ctx.send(
                f"guild > {guild.name}, inv > {await self.generate_invite(guild)}"
            )
        else:
    ...
    @commands.command(
        name="leaveguild",
        description="Force the bot to leave a guild by its ID.",
        permission="Owner",
    )
    @commands.is_owner()
    async def leaveguild(self, ctx: Context, guild_id: int):
        ...
        if guild:
            await guild.leave()
            await ctx.approve(
                f"Successfully left the guild: {guild.name} (ID: {guild.id})"
            )
        else:
            await ctx.warn(f"Guild with ID {guild_id} not found.")

    @commands.command(
        name="globalban", aliases=["gban"], description="Global bans a user."
    )
    @commands.is_owner()
    async def globalban(self, ctx: Context, *, user: discord.User):
        ...
        if user.id == self.bot.user.id:
            return await ctx.deny(
                f"Bro tried global banning me from {len(self.bot.guilds)} 💀"
            )
    ...
    @command(name="whitelist", aliases=["wl", "auth"], description="Authorize a guild.")
    @is_owner()
    ...
    @premium.command(name="add", aliases=["give"], description="Give a user premium.")
    @is_owner()
    async def premium_give(
        ...
        await self.bot.pool.execute(
            "INSERT INTO premium (user_id) VALUES ($1)", user.id
        )
        return await ctx.approve(f"**{user.name}** has been **granted** premium.")

    @premium.command(
        name="remove", aliases=["take", "revoke"], description="Revokes a user premium."
    )
    @is_owner()
    ...
    async def apikey_add(self, ctx: Context, user: discord.User, role: str):
        ...
        check = await self.bot.pool.fetchrow(
            "SELECT * FROM api_key WHERE user_id = $1", user.id
        )
        ...
        embed = discord.Embed(
            description=f"Your API key for {url} is listed above.",
            color=Colors.BASE_COLOR,
        )

        await self.bot.pool.execute(
            "INSERT INTO api_key (key, user_id, role) VALUES ($1, $2, $3)",
            key,
            user.id,
            role,
        )
    ...
    async def push(self, ctx: Context):
        try:
            ...
            await ctx.approve(
                f"```{result.stdout}{commit_result.stdout}{push_result.stdout}```"
            )
