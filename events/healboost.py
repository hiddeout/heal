...
class healboost(Cog):
    ...
    async def on_member_boost(self, before: discord.Member, after: discord.Member):
        if before.guild.id != 1:  # edit with your own guild
            return
        if (
            before.guild.premium_subscriber_role in before.roles
            or after.guild.premium_subscriber_role not in after.roles
        ):
            return
        data = await self.bot.pool.fetchrow(
            "SELECT user_id FROM premium WHERE user_id = $1", before.id
        )
        if data:
            return
        await self.bot.pool.execute(
            "INSERT INTO premium (user_id) VALUES ($1)", before.id
        )
        role = before.guild.get_role()  # edit with your own role
        await before.add_roles(role)
        embed = discord.Embed(
            description=f"> you've been granted premium for the duration of your boost(s), when you unboost, your premium will be automatically removed",
